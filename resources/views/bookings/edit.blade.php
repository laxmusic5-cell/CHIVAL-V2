                    ...
                    <form method="POST" action="{{ route('bookings.update', $booking) }}">
                        ...
                        <div class="grid gap-6">
                            <div>
                                ...
                                <select id="customer_id" name="customer_id" class="mt-1 block w-full rounded-md border-gray-300 bg-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    ...
                                    @foreach ($customers as $customer)
                                        <option value="{{ $customer->id }}" {{ old('customer_id', $booking->customer_id) === $customer->id ? 'selected' : '' }}>{{ $customer->name }} ({{ $customer->email }})</option>
                                    @endforeach
                                </select>
                                @error('customer_id')
                            ...
                            <div>
                                ...
                                <select id="customer_vehicle_id" name="customer_vehicle_id" class="mt-1 block w-full rounded-md border-gray-300 bg-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    ...
                                    @foreach ($customerVehicles as $cv)
                                        <option value="{{ $cv->id }}" {{ old('customer_vehicle_id', $booking->customer_vehicle_id) === $cv->id ? 'selected' : '' }}>{{ $cv->brand }} {{ $cv->model }} — {{ $cv->plate_number }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="coverage_area_id" name="coverage_area_id" class="mt-1 block w-full rounded-md border-gray-300 bg-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    ...
                                    @foreach ($coverageAreas as $area)
                                        <option value="{{ $area->id }}" {{ old('coverage_area_id', $booking->coverage_area_id) === $area->id ? 'selected' : '' }}>{{ $area->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input id="booking_date" name="booking_date" value="{{ old('booking_date', optional($booking->booking_date)->format('Y-m-d')) }}" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                @error('booking_date')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input id="booking_time_slot" name="booking_time_slot" value="{{ old('booking_time_slot', $booking->booking_time_slot) }}" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            ...
                            <div>
                                ...
                                <input id="address" name="address" value="{{ old('address', $booking->address) }}" type="text" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                @error('address')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea id="notes" name="notes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('notes', $booking->notes) }}</textarea>
                                @error('notes')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
