<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Add Booking Item</h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('booking-items.store') }}">
                        @csrf
                        <div class="grid gap-6">
                            <div>
                                ...
                                <select id="booking_id" name="booking_id" class="mt-1 block w-full rounded-md border-gray-300 bg-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    ...
                                    @foreach ($bookings as $b)
                                        <option value="{{ $b->id }}" {{ old('booking_id') === $b->id ? 'selected' : '' }}>{{ $b->code }} — {{ optional($b->booking_date)->format('Y-m-d') }}</option>
                                    @endforeach
                                </select>
                                @error('booking_id')
                            ...
                            <div>
                                ...
                                <select id="addon_id" name="addon_id" class="mt-1 block w-full rounded-md border-gray-300 bg-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach ($addons as $a)
                                        <option value="{{ $a->id }}" {{ old('addon_id') === $a->id ? 'selected' : '' }}>{{ $a->name }}</option>
                                    @endforeach
                                </select>
                                @error('addon_id')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <input id="quantity" name="quantity" value="{{ old('quantity', 1) }}" type="number" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                @error('quantity')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input id="service_price_snapshot" name="service_price_snapshot" value="{{ old('service_price_snapshot', 0) }}" type="number" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            ...
                            <div>
                                ...
                                <input id="addon_price_snapshot" name="addon_price_snapshot" value="{{ old('addon_price_snapshot', 0) }}" type="number" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <div>
                                ...
                                <input id="subtotal" name="subtotal" value="{{ old('subtotal', 0) }}" type="number" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                @error('subtotal')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
